...
- 오래된 lead thread (5/6 기준 30일+ 전) 같은 영역이 14일 윈도우엔 누락 → 6번 LLM이 *최근 정체*만 보고 mid 판정 → 옵션 C로 흡수되지만 *진짜 raw 보고 정밀 판단* 가치 있음
...
**왜 lead_history 별도 영역?** — 5번 소통 점수와 6번 거래 의지 LLM 영역 분리 (스킬 3 STEP 1 영역 분리 참조). 오래된 lead thread 같은 *영업 사이클 시작 시점* raw가 14일 윈도우엔 누락. 6번 LLM이 *진짜 적극도 판단* 가능하게 lead_history를 명시 입력으로 박음.
...
## STEP 4: LLM 2번 — reason_headline + reason + next_action 일괄 생성 (5/6 갱신)

### 출력 3 영역 (5/6 신규)

| 필드 | 영역 | 적용 티어 |
|---|---|---|
| `reason_headline` | 🎯 한 줄 통찰 (변곡점·임박·핵심 시그널) | **T1 한정** (T2·T3 null) |
| `reason` | 데이터 근거 (시점·메시지 인용) | T1·T2·T3 모두 |
| `next_action` | 시점 + 행동 + 효과/근거 풀어쓰기 | T1·T2·T3 모두 |

### 길이·톤 원칙

| 티어 | reason_headline | reason | next_action |
|---|---|---|---|
| **T1** (≥87) | 50~80자, 결론 + 핵심 시그널 1~2개 | 최대 3줄 (핵심 팩트 + 임팩트) | 100~150자 (시점 + 행동 + 효과) |
| **T2** (≥55) | null (헤드라인 X) | 최대 2줄 (상태 + 병목) | 50~100자 |
| **T3** (<55) | null | 1줄 (간결 상태) | 30~60자 |

### LLM 프롬프트 (5/6 갱신)

```
당신은 B2B 영업 딜 우선순위 리포트를 작성하는 어시스턴트입니다.
scored_deals.json 전체를 읽고 N건의 reason_headline·reason·next_action을 한 응답으로 일괄 반환하세요.

## reason_headline 작성 원칙 (T1 한정, 5/6 신규)

T1 영역에 *🎯 한 줄 통찰*로 표시되는 영역입니다. LD가 결과물 보자마자 "왜 이 딜이 즉시 대응 영역인지"를 한 호흡에 인지하게 하는 핵심 영역.

1. **결론 + 핵심 시그널** — 결론 문장 1개 + 핵심 시그널 1~2개를 대시(—)·콤마로 연결
   - 좋은 예: "5/8 데모 만료 + 5/4 고객 '도입 절차·가격 조율' 요청 — 이번 주가 계약 결정 변곡점. 인원 70명·구독형 합의 후 가격만 남은 단계"
   - 좋은 예: "이번 주 안에 결정 — 강사 조건 확정 + 고객 '금주 회신' 약속. 두 과정 동시 진행 흐름"
   - 나쁜 예: "고객이 적극적이고 진행 잘 됨" (추상)

2. **길이**: 50~80자 (한 줄 안에 핵심)

3. **직관적 한국어** — 영어 차용어(*모멘텀·픽업·트랙션* 등) 회피, *변곡점·성사 가능성·결정 시점·진행 흐름* 같은 직관적 한국어 사용

4. **시점·시간 강조** — "이번 주" / "5/8까지" / "5월 중" 같은 영업 시점 박음

5. **T2·T3는 null** — 헤드라인 영역 X (T2·T3는 한 줄 reason으로 충분)
...
4. **헤드라인과 중복 X** (5/6 신규) — reason은 *데이터 근거 (시점·메시지 인용)*. 헤드라인이 *결론*이라면 reason은 *근거*. 같은 표현 반복 금지.

5. **티어별 톤:**

   **T1 reason (집중)** — "왜 지금 즉시 대응해야 하는가":
   - 길이: 최대 3줄
   - 구조: [핵심 팩트 2~3개 — 시점·메시지 인용] + [비즈니스 임팩트 판단]
   - 예: "4/22~4/27 양방향 메일·슬랙 활발, 4/27 강사 조건 확정 견적서 진행. 고객 '금주 중 의사결정 후 회신' 약속 + 능동 요구사항 추가. 두 과정 동시 진행 흐름 — 이번 주 내 결정 시 양쪽 성사 가능성 높음."
...
6. **금지 표현:** 대화체("~죠"·"~네요") / 추측성("아마"·"~일 수도") / 뱃지 반복 / 주관 형용사("좋은"·"훌륭한") / 영어 차용어("모멘텀"·"트랙션" 등) — 직관적 한국어 우선

## next_action 작성 원칙 (5/6 풀어쓰기 가이드)

T1 영역에 ⚡ 추천 액션으로 표시. **시점 + 행동 + 효과/근거** 3 영역을 한 호흡에 박음.

1. **T1 — 풀어쓰기 (100~150자)**:
   - 구조: **시점 (bold)** + 구체 행동 + → 효과/근거
   - 좋은 예: "**5/7~5/8 (데모 만료 전)**: 도입 절차·계약 안내 회신 + 인원 70명·구독형 가격 확정안 송부. 인당 월 30만원 한도 내 단가 정리 → 고객 '5월 중 의사결정' 약속을 이번 주 안으로 끌어옴."
   - 좋은 예: "**5/8~5/12**: 임원 과정 ASAP 견적 송부 + 중간관리자 리더십 견적 회신 받기. 두 과정을 *6월 동시 실행* 타깃으로 묶어 제안 → 고객이 한 번에 두 건 의사결정 시 양쪽 성사 가능성 높임."

2. **T2 — 50~100자**: 시점 + 행동 (효과는 짧게)
   - 예: "5/7~5/9 강사 레퍼런스 + 6시간 커리큘럼 수정안 회신."

3. **T3 — 30~60자**: 행동만 한 줄
   - 예: "5/12 전후 1회 f-up 후 LOST 판정 검토."

4. **추상 표현 금지** — "팔로업 필요"·"검토" 단독 X. 구체 행동·시점 명시.

## 출력 형식 (JSON 배열, 5/6 갱신)

[
  {
    "deal_id": "019baba5-...",
    "reason_headline": "...",   // T1만 작성, T2·T3는 null
    "reason": "...",
    "next_action": "..."
  },
  ...
